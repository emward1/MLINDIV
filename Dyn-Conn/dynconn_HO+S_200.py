...
def extract_fMRI_into_atlas(sub, path, pattern, scan_num, confounds_directory, atlas_dict, output_label, output_directory, cols_to_remove = []):
    ...
    scan_names = glob.glob(directory_path + "/*" + pattern + "*preproc_bold.nii.gz")
...
#def save_3D_array(binthresh_connectomes_dict, sub, z, output_directory, output_label, pattern):
...
confounds_directory = "/mnt/chrastil/lab/users/erica/projects/MLINDIV/githubclone/Dyn-Conn/confounds/"
timeseries_directory = "/mnt/chrastil/lab/users/erica/projects/MLINDIV/Timeseries"
array_directory = "/mnt/chrastil/lab/users/erica/projects/MLINDIV/Arrays/S200"

# Loop pattern
# pattern = "Ex", scan_num =2
# pattern = "Run", scan_num=6
#pattern = "Ex"
#scan_num = 2
threshold = 0
window_step = 130
...
# HO + Schaeffer Atases
output_label = "HOsubcort+Schaefer200_2mm"
# ts_pattern = "*Ex*HOsubcort+Schaefer200_2mm*"

schaefer = datasets.fetch_atlas_schaefer_2018(n_rois = 200, resolution_mm=2, verbose = 0)
...
f = open("/mnt/chrastil/lab/users/erica/projects/MLINDIV/githubclone/Dyn-Conn/roi_labels214.txt", 'rb')
roi_names = pickle.load(f)
f.close()


def loop_thru_scan_types(extract_fMRI_into_atlas, bin_time_series, connectomes_from_time_windows, binarize_threshold):
    scan_type = ["Ex", "Run"]
    scan_nums=(2, 6)
    ts_pattern =["*Ex*HOsubcort+Schaefer200_2mm*", "*Run*HOsubcort+Schaefer200_2mm*"]
    for (pattern, scan_num, ts_patt) in zip(scan_type, scan_nums, ts_pattern):
        extract_fMRI_into_atlas(sub, path, pattern, scan_num, confounds_directory, atlas_dict, output_label, timeseries_directory, cols_to_remove = [200, 201, 202, 207, 211, 212, 213])
        time_windows = bin_time_series(sub, timeseries_directory, ts_patt, window_step, window_size)
        ...
        scipy.io.savemat('%s/sub-%s_%s_%s_3Darray.mat' % (array_directory, sub, pattern, output_label), binthresh_connectomes, long_field_names = True)
        print("\n\n---Saved 3D Array to %s/sub-%s_%s_%s_3Darray.mat---" % (array_directory, sub, pattern, output_label))

        #save_3D_array(binthresh_connectomes, sub, pattern, array_directory, output_label)

# Code to execute w/ above parameters
if __name__ == "__main__":
	print("Executing dynconn_HO+S_200.py on subject: " + sub)
	#print("===PARAMETERS===\nSubject: %s\nPath: %s\nConfounds Directory: %s\nTimeseries Directory: %s\n3D Array Directory: %s\nBinarize threshold: %f\nOutput Label: %s\nAtlases:\n" % (sub, path, confounds_directory, timeseries_directory, array_directory, threshold, output_label))
	for atlas_file, maskertype in atlas_dict.items():
		print("\t%s\t\tNiftiMasker Used: %s" % (atlas_file, maskertype))
loop_thru_scan_types(extract_fMRI_into_atlas, bin_time_series, connectomes_from_time_windows, binarize_threshold)